...
cat("RMSE: ", 
    RMSE = sqrt(sum(residuals(categorical_lm) ^ 2) / df.residual(categorical_lm))
)
...
* To interpret the coefficients above, Let the categorical variable $x$ be into a dummy variable which takes values `(0, 1, 2, 3)` for `('a', 'b', 'c', 'd')` correspondingly,
    * average $y$ is higher by 6.9639 units for `b` than for `a`
    * average $y$ is higher by 4.9750 units for `c` than for `a`
    * average $y$ is higher by 0.7588 units for `d` than for `a`
